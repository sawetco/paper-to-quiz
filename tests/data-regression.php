...
if (getenv('PTQ_ALLOW_REGRESSION') !== '1') {
	throw new RuntimeException('Set PTQ_ALLOW_REGRESSION=1 to enable destructive regression checks.');
}

/**
 * @throws RuntimeException When a regression assertion fails.
 */
function ptq_assert(bool $condition, string $message): void {
	if (! $condition) {
 ...
 */
function ptq_cleanup_counts(wpdb $wpdb, Database $database, string $suffix): array {
	$like = '%' . $wpdb->esc_like($suffix) . '%';

	return array(
		'ptq_assessments' => (int) $wpdb->get_var(
			$wpdb->prepare(
...
try {
	...
	ptq_assert($administrator instanceof WP_Role, 'Administrator role could not be loaded.');
	foreach (array('ptq_manage_assessments', 'ptq_publish_assessments', 'ptq_view_results', 'ptq_manage_settings') as $capability) {
		ptq_assert($administrator->has_cap($capability), 'Administrator role is missing ' . $capability . '.');
	}

	$created_class = $service->save_class('Regression Class ' . $suffix, null, '#2f6fed');
	ptq_assert(! is_wp_error($created_class), 'Class could not be created.');
	$class_id = (int) $created_class['id'];
	ptq_assert($created_class['color'] === '#2f6fed', 'Class color was not persisted.');
	ptq_assert($service->trash_class($class_id), 'Class could not be moved to trash.');

	$restored_class = $service->save_class('Regression Class ' . $suffix, null, '#d63638');
	ptq_assert(! is_wp_error($restored_class), 'Trashed class could not be recreated.');
	ptq_assert((int) $restored_class['id'] === $class_id, 'Recreating a trashed class did not reuse its identity.');
	ptq_assert($restored_class['status'] === 'active', 'Recreated class was not restored.');
	ptq_assert($restored_class['color'] === '#d63638', 'Restored class color was not updated.');

	$class_conflict = $service->save_class('Regression Class ' . $suffix, null, '#1769aa');
	ptq_assert(is_wp_error($class_conflict), 'An active duplicate class was accepted.');
	ptq_assert($class_conflict->get_error_code() === 'ptq_term_exists', 'Active duplicate class did not return a conflict.');
	ptq_assert((int) $class_conflict->get_error_data()['status'] === 409, 'Active duplicate class did not return HTTP 409.');

	$created_subject = $service->save_subject('Regression Subject ' . $suffix);
	ptq_assert(! is_wp_error($created_subject), 'Subject could not be created.');
	$subject_id = (int) $created_subject['id'];
	ptq_assert($service->trash_subject($subject_id), 'Subject could not be moved to trash.');

	$restored_subject = $service->save_subject('Regression Subject ' . $suffix);
	ptq_assert(! is_wp_error($restored_subject), 'Trashed subject could not be recreated.');
	ptq_assert((int) $restored_subject['id'] === $subject_id, 'Recreating a trashed subject did not reuse its identity.');
	ptq_assert($restored_subject['status'] === 'active', 'Recreated subject was not restored.');
	...
	foreach (array('exam', 'test') as $index => $type) {
		...
		ptq_assert($inserted === 1, 'Regression assessment could not be inserted.');
		...
		ptq_assert($inserted === 1, 'Regression revision could not be inserted.');
	...
	ptq_assert(! is_wp_error($bootstrap), 'Published regression test could not be bootstrapped.');
	ptq_assert($bootstrap['class_color'] === '#d63638', 'Student bootstrap did not receive the selected class color.');

	ptq_assert($service->trash($assessment_a), 'Target exam could not be moved to trash.');
	$purged = $service->purge($assessment_a);
	ptq_assert(! is_wp_error($purged), 'Target exam could not be permanently deleted.');
	ptq_assert(
	...
	ptq_assert(! is_wp_error($repeated_purge), 'Repeating a completed permanent deletion returned an error.');
	ptq_assert(! empty($repeated_purge['already_deleted']), 'Repeated permanent deletion was not reported as idempotent.');
	...
	foreach (array_filter(array($revision_a, $revision_b)) as $revision_id) {
		ptq_assert(false !== $wpdb->delete($database->table('questions'), array('revision_id' => $revision_id), array('%d')), 'Regression questions could not be cleaned up.');
	}
	foreach (array_filter(array($assessment_a, $assessment_b)) as $assessment_id) {
		ptq_assert(false !== $wpdb->delete($database->table('revisions'), array('assessment_id' => $assessment_id), array('%d')), 'Regression revisions could not be cleaned up.');
		ptq_assert(false !== $wpdb->delete($database->table('assessments'), array('id' => $assessment_id), array('%d')), 'Regression assessments could not be cleaned up.');
	}
	if ($subject_id) {
		ptq_assert(false !== $wpdb->delete($database->table('terms'), array('id' => $subject_id, 'type' => 'subject'), array('%d', '%s')), 'Regression subject could not be cleaned up.');
	}
	if ($class_id) {
		ptq_assert(false !== $wpdb->delete($database->table('terms'), array('id' => $class_id, 'type' => 'class'), array('%d', '%s')), 'Regression class could not be cleaned up.');
	}
	$cleanup_counts = ptq_cleanup_counts($wpdb, $database, $suffix);
	ptq_assert($cleanup_counts === array('ptq_assessments' => 0, 'ptq_revisions' => 0, 'ptq_questions' => 0, 'ptq_terms' => 0), 'Regression cleanup left owned records behind.');
}
